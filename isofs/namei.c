 ...
 */
static int isofs_name_match(const char *name1, size_t len1, const char *name2, size_t len2)
{
 ...
 */
static struct buffer_head_t *isofs_find_entry(struct inode_t *dir, const char *name, size_t name_len,
                                              ino_t *res_ino, ino_t *backlink_ino)
{
...
int isofs_lookup(struct inode_t *dir, const char *name, size_t name_len, struct inode_t **res_inode)
{
  ...
  *res_inode = vfs_iget(dir->i_sb, ino);
  ...
  /* save backlink */
  if (backlink)
    isofs_i(dir)->i_backlink = backlink;
